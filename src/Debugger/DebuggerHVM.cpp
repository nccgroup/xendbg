...
Address DebugSessionPV::single_step() {
  ...
  bool dest1_had_il = dest1_addr && (_breakpoints.count(*dest1_addr) != 0);
  bool dest2_had_il = dest2_addr && (_breakpoints.count(*dest2_addr) != 0);
...
std::vector<Address> DebugSessionPV::get_breakpoints() {
  ...
  std::transform(_breakpoints.begin(), _breakpoints.end(),
...
void DebugSessionPV::insert_breakpoint(Address address) {
  ...
  const auto mem_handle = get_domain().map_memory<char>(address, 1, PROT_READ | PROT_WRITE);
  const auto mem = (uint8_t*)mem_handle.get();

  const auto orig_byte = *mem;

  _breakpoints[address] = orig_byte;
  *mem = X86_DEBUG_BREAKPOINT;
}
...
void DebugSessionPV::remove_breakpoint(Address address) {
  ...
  const auto mem_handle = get_domain().map_memory<char>(address, 1, PROT_WRITE);
  const auto mem = (uint8_t*)mem_handle.get();

  const auto orig_byte = _breakpoints.at(address);
  *mem = orig_byte;
...
DebugSessionPV::read_memory_masking_breakpoints(Address address, size_t length) {
  ...
  for (const auto [il_address, il_orig_bytes] : _breakpoints) {
...
void DebugSessionPV::write_memory_retaining_breakpoints(Address address, size_t length, void *data) {
  ...
  for (const auto [il_address, _] : _breakpoints) {
    if (il_address >= address && il_address < address_end) {
