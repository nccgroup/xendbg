...
UVSignal::UVSignal(const UVLoop &loop)
  : _loop(loop), _signal(new uv_signal_t), _is_running(false)
{
...
UVSignal::~UVSignal() {
  uv_close(uv_upcast<uv_handle_t>(_signal), [](uv_handle_t *close_handle) {
    free(close_handle);
  });
}
